...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		// txtTracklist.setFormat("assets/fonts/vcr.ttf", 32);
		...
		var selectSomeone:FlxText = new FlxText(0,0,0,"Select a character");
	...
	function changeCharacter(dir:Int = 0)
	{
		...
		charName.text = Main.characterNames[selectedChar];
		charName.updateHitbox();
		charName.screenCenter(X);
		charAbility.text = "Special Ability:\n";
		charAbility.text += Main.characterSpecials[selectedChar];
		charAbility.updateHitbox();
		charAbility.screenCenter(X);
		if (Main.characterCredits[selectedChar] == "")
			charCredit.text = "";
		else
			charCredit.text = "ft. " + Main.characterCredits[selectedChar];
